                ...
                foreach (var item in movies)
                {
                    foreach (var scraper in _scraperManager.All())
                    {
                        try
                        {
                            ...
                            if (media != null)
                            {
                                // mediaId 是搜索时用的 season_id/media_id (例如 '41175')
                                // media.CommentId 是 GetMedia 解析出的实际 ep_id (例如 '470296' 或 '791639')
                                string actualPlayableEpId = media.CommentId; 
                                if (string.IsNullOrEmpty(actualPlayableEpId) && media.Episodes.Any())
                                {
                                    actualPlayableEpId = media.Episodes.First().CommentId; // 备选，确保拿到一个ep_id
                                }

                                _logger.LogInformation("[{0}]匹配成功：name={1} ProviderId: {2}, CommentId={3}", scraper.Name, item.Name,
                                    mediaId, actualPlayableEpId); // 日志中显示实际用于弹幕的ID

                                // 更新epid元数据
                                item.SetProviderId(scraper.ProviderId, mediaId); // 仍然保存搜索时用的 mediaId (如 '41175') 作为主关联
                                // 可以考虑额外存储一个特定于播放的 ep_id，如果 Emby 支持多个 ProviderId 或自定义字段
                                // 例如: item.SetProviderId($"{scraper.ProviderId}_ep", actualPlayableEpId);
                                item.UpdateToRepository(ItemUpdateType.MetadataEdit);

                                // 下载弹幕
                                if (!string.IsNullOrEmpty(actualPlayableEpId)) {
                                    await this.DownloadDanmu(scraper, item, actualPlayableEpId).ConfigureAwait(false);
                                } else {
                                    _logger.Warn($"[{scraper.Name}]为电影 '{item.Name}' (B站ID: {mediaId}) 未能从GetMedia结果中确定有效的ep_id用于下载弹幕。");
                                }
                    ...
                    foreach (var scraper in _scraperManager.All())
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(providerVal))
                            {
                                ...
                                if (media != null)
                                {
                                    string actualEpIdForEpisodeMethod = media.CommentId; // 通常是第一个版本的 ep_id
                                    if (string.IsNullOrEmpty(actualEpIdForEpisodeMethod) && media.Episodes.Any())
                                    {
                                        actualEpIdForEpisodeMethod = media.Episodes.First().CommentId;
                                    }

                                    if (!string.IsNullOrEmpty(actualEpIdForEpisodeMethod))
                                    {
                                        var episodeDetails = await scraper.GetMediaEpisode(item, actualEpIdForEpisodeMethod);
                                        if (episodeDetails != null && !string.IsNullOrEmpty(episodeDetails.CommentId))
                                        {
                                            _logger.LogInformation("[{0}]为电影 '{1}' (B站ID: {2}, 使用ep_id: {3}) 成功获取剧集信息，最终CommentId: {4}", scraper.Name, item.Name, providerVal, actualEpIdForEpisodeMethod, episodeDetails.CommentId);
                                            await this.DownloadDanmu(scraper, item, episodeDetails.CommentId).ConfigureAwait(false);
                                        } else
                                        {
                                             _logger.Warn($"[{scraper.Name}]为电影 '{item.Name}' (B站ID: {providerVal}, 尝试的ep_id: {actualEpIdForEpisodeMethod}) 调用 GetMediaEpisode 返回了 null 或无效CommentId。");
                                        }
                                    } else {
                                        _logger.Warn($"[{scraper.Name}]为电影 '{item.Name}' (B站ID: {providerVal}) 调用 GetMedia 后未能确定有效的 ep_id。");
                                    }
                                } else {
                                    _logger.Warn($"[{scraper.Name}]为电影 '{item.Name}' (B站ID: {providerVal}) 调用 GetMedia 返回了 null。");
                                }
